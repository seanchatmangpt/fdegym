...
HERE=pathlib.Path(__file__).resolve().parent; SUBJECT=json.loads((HERE/'subject.json').read_text()); CASES=json.loads((HERE/'cases.json').read_text()); HEX40=re.compile(r'^[0-9a-f]{40}$'); EXPECTED='seanchatmangpt/fdegym'
def classify(s):
...
def main():
    ...
    print('R85_CONSUMER='+standing); print('SUBJECT_DIGEST='+hashlib.sha256(json.dumps(SUBJECT,sort_keys=True,separators=(',',':')).encode()).hexdigest()); print('CASE_COUNT='+str(len(CASES)))
    if failures: print('REFUSED[R85_COURT]='+','.join(failures)); return 1
    return 0
